...
| File | Responsibility |
|---|---|
| `js/i18n.js` | `L`, `STR{tr,en}` (310 keys each, must stay equal), `NEWS` templates, `t()`, link helpers |
...
- **Every user-visible string is bilingual.** Add to both `STR.tr` and `STR.en`; the
  counts must match (310 today). Objects returned from events, themes, branches and
